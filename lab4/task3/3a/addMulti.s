...
x_struct: dd 5
	x_num: db 0xff, 0xff, 0xff, 0xff, 0
y_struct: dd 4
	y_num: db 2,0,0,0
...
; y_struct: dd 6
; 	y_num: db 0xaa, 1, 2, 3, 0xcc, 0x4f

; x_struct: dd 5
; 	x_num: db 0xaa, 1, 2, 0xcc, 0x4f
; y_struct: dd 5
; 	y_num: db 0xaa, 1, 2, 0xcc, 0x4f
...
add_multi:
	...
	mov dword ebx, [ebp - 4]	; ebx points to struct1
	mov ebx, [ebx]		; len1
	mov [ebp - 12], ebx		; VAR1 = len1
	...
	mov ebx, [ebp - 12]	; ebx = VAR1 = len1
	mov [eax], ebx		; newStruct.len = ebx = len1

	mov dword ebx, [ebp - 8]	; ebx points to struct2
	mov ebx, [ebx]				; ebx = len2
	sub dword [ebp - 12], ebx	; VAR1 = len1 - len2
	mov dword [ebp - 16], ebx	; VAR2 = len2
	...
	sbb  dh, dh
	and  dh, 0x01 ; dl will hold carry

	mov edi, [ebp - 12]	; edi = var1 = len1 - len2
	; inc edi
	cmp edi, 0
	je end2
