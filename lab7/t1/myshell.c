...
void execute(cmdLine *pCmdLine, process **processList) {
	...
	if((pid=fork()) == 0) {
		// write(STDOUT_FILENO, "stdin: ", strlen("stdin: "));
		// write(STDOUT_FILENO, pCmdLine->inputRedirect, strlen(pCmdLine->inputRedirect));
		// write(STDOUT_FILENO, "\n", 1);

		// write(STDOUT_FILENO, "stdout: ", strlen("stdout: "));
		// write(STDOUT_FILENO, pCmdLine->outputRedirect, strlen(pCmdLine->outputRedirect));
		// write(STDOUT_FILENO, "\n", 1);
