...
void parseArguments(Config* config, int argc, char* argv[]) {
	...
	for (i = 1; i < argc; ++i) {
		if (argv[i][1] == 'e') {
			...
			config->inputFileDescriptor = fopen(argv[i] + 2, "r");
		}
		else if (argv[i][0] == '-' && argv[i][1] == 'o') {
			config->outputFileDescriptor = fopen(argv[i] + 2, "w");
		}
...
int main(int argc, char* argv[]) {
	...
	Config config = {0, stdin, stdout, {0, NULL}};
	...
	if (config.inputFileDescriptor == NULL) {
		fputs("The file couldn't be opened for writing.\n", stderr);
		return -1;
	}

	while ( (inputChar = fgetc(config.inputFileDescriptor)) != EOF ) {
		encodedChar = encode(&config, inputChar);

		fputc(encodedChar, config.outputFileDescriptor);
	}
